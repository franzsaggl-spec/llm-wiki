# Index

Catalog of wiki pages.

## Sources
- [wiki/source-claude-code-docs-full.md](wiki/source-claude-code-docs-full.md) — Combined Claude Code docs source (80 markdown pages) with first-pass synthesis.
- [wiki/source-example.md](wiki/source-example.md) — Placeholder source summary.

## Concepts
- [wiki/concept-claude-code-architecture.md](wiki/concept-claude-code-architecture.md) — High-level architecture model from docs.
- [wiki/concept-claude-code-operations.md](wiki/concept-claude-code-operations.md) — Operational workflow model (interactive + automated).
- [wiki/concept-example.md](wiki/concept-example.md) — Placeholder concept page.

## Entities
- *(none yet)*

## Analyses
- [wiki/analysis-plugin-sharing-at-scale.md](wiki/analysis-plugin-sharing-at-scale.md) — Recommended rollout and governance model for sharing plugins to ~300 developers.
